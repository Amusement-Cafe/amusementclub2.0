...
cmd(['leaderboard', 'tomatoes', 'local'], withInteraction(async (ctx, user, args) => {
    let embed = {
        color: colors.blue
    }
...
cmd(['leaderboard', 'vials'], withInteraction(async (ctx, user, args) => {
    ...
    if (!topVials.some(x => x.discord_id === user.discord_id) && user.vials)
        table.addRow(`${greater+1}`, user.username, numFmt(Math.floor(user.vials)))
    embed.description = `\`\`\`${table.toString()}\`\`\``
    return ctx.interaction.createFollowup({embeds: [embed]})
}))

cmd(['leaderboard', 'vials', 'local'], withInteraction(async (ctx, user, args) => {
    let embed = {
        color: colors.blue
    }
    const guildUsers = await getGuildUsers(ctx)
...
cmd(['leaderboard', 'lemons'], withInteraction(async (ctx, user, args) => {
    ...
    if (!topLemons.some(x => x.discord_id === user.discord_id) && user.lemons)
        table.addRow(`${greater+1}`, user.username, numFmt(Math.floor(user.lemons)))
    embed.description = `\`\`\`${table.toString()}\`\`\``
    return ctx.interaction.createFollowup({embeds: [embed]})
}))

cmd(['leaderboard', 'lemons', 'local'], withInteraction(async (ctx, user, args) => {
    let embed = {
        color: colors.blue
    }
    const guildUsers = await getGuildUsers(ctx)
...
cmd(['leaderboard', 'cards', 'local'], withInteraction(async (ctx, user, args) => {
    let embed = {
        color: colors.blue
    }
    const guildUsers = await getGuildUsers(ctx)
...
cmd(['leaderboard', 'clout', 'local'], withInteraction(async (ctx, user, args) => {
    let embed = {
        color: colors.blue
    }
    const guildUsers = await getGuildUsers(ctx)
...
cmd(['leaderboard', 'completed', 'local'], withInteraction(async (ctx, user, args) => {
    let embed = {
        color: colors.blue
    }
    const guildUsers = await getGuildUsers(ctx)
...
cmd(['leaderboard', 'level', 'local'], withInteraction(async (ctx, user, args) => {
    let embed = {
        color: colors.blue
    }
    const guildUsers = await getGuildUsers(ctx)
